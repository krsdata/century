...
$qry="SELECT `contests_id`, `amount`, `no_of_teams`, `fee`,type,jinku FROM contests cc where  matchid='$id1' order by fee";
...
while($rows=mysqli_fetch_assoc($result))
{
  ...
  $fee=$rows['fee'];
  $bonus=$rows['jinku'];
  $bonus_min=($fee*$no_of_teams*$bonus)/100;
$winners=0;
$type11=$rows['type'];
...
while($rows1=mysqli_fetch_assoc($result1))
{
 ...
 /* $qry2y="SELECT code1 FROM `contest11` where  contestid='$contest_id'";
$result2y=mysqli_query($conn,$qry2y);
$rows2y=mysqli_num_rows($result2y);
if($rows2y>0)
{
...
{ */
$ctccount = 0;  
$qry2="SELECT count(*) as cont1 FROM `matches_joined` where  contests_id='$contest_id'  and  `match_id`='$id1'";
...
$qryert="SELECT * FROM prize where contestid='$contest_id'";
$resultert = mysqli_query($conn,$qryert);
$rowsert = mysqli_num_rows($resultert);
   ...
   $rt=$rowert['max1'];
    $rt1=$rowert['min1'];
     $rt2=$rowert['prize'];
       ...
       if($rt>$rt1)
     $sum=$sum+(($rt-$rt1+1)*$rt2);
     else if($rt1>$rt)
     $sum=$sum+(($rt1-$rt+1)*$rt2);
     else
      $sum=$sum+((1)*$rt2);
...
$cntu3=$no_of_teams*$fee;

$contu1234=$cntu3-$contu123;


$sum1111=$sum1111+$contu1234-$bonus_min;
...
$ty1=((intval($contest_id)+$ty)*3)-$ty;
  ...
  echo '
<a href="contests4_new.php?trans1='.$contest_id.'&trans2='.$match_id.'">
